...
export default function RecognitionPage() {
    ...
    if (!sign || !sign.recognitionText)
        return _jsx(Navigate, { to: MainRouter.MainPage });
    const signText = `${sign.text[0].toUpperCase()}${sign.text.slice(1)}`;
    return (_jsx(SignRecognitionBlock, { word: signText, className: "h-fit m-0 flex-grow", recognitionText: sign.recognitionText }));
}
